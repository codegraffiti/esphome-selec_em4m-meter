#include "selec_em4m_meter.h"
#include "selec_em4m_meter_registers.h"
#include "esphome/core/log.h"

namespace esphome {
namespace selec_em4m_meter {

static const char *const TAG = "selec_em4m_meter";

static const uint8_t MODBUS_CMD_READ_IN_REGISTERS = 0x04;
static const uint8_t MODBUS_REGISTER_COUNT = 34;  // 34 x 16-bit registers

void Selecem4mMeter::on_modbus_data(const std::vector<uint8_t> &data) {
  if (data.size() < MODBUS_REGISTER_COUNT * 2) {
    ESP_LOGW(TAG, "Invalid size for Selecem4mMeter!");
    return;
  }

  auto selec_em4m_meter_get_float = [&](size_t i, float unit) -> float {
  ...
  float total_active_energy = selec_em4m_meter_get_float(SELEC_TOTAL_ACTIVE_ENERGY * 2, NO_DEC_UNIT);
  float import_active_energy = selec_em4m_meter_get_float(SELEC_IMPORT_ACTIVE_ENERGY * 2, NO_DEC_UNIT);
  float export_active_energy = selec_em4m_meter_get_float(SELEC_EXPORT_ACTIVE_ENERGY * 2, NO_DEC_UNIT);
  float total_reactive_energy = selec_em4m_meter_get_float(SELEC_TOTAL_REACTIVE_ENERGY * 2, NO_DEC_UNIT);
  float import_reactive_energy = selec_em4m_meter_get_float(SELEC_IMPORT_REACTIVE_ENERGY * 2, NO_DEC_UNIT);
  float export_reactive_energy = selec_em4m_meter_get_float(SELEC_EXPORT_REACTIVE_ENERGY * 2, NO_DEC_UNIT);
  float apparent_energy = selec_em4m_meter_get_float(SELEC_APPARENT_ENERGY * 2, NO_DEC_UNIT);
  float active_power = selec_em4m_meter_get_float(SELEC_ACTIVE_POWER * 2, MULTIPLY_THOUSAND_UNIT);
  float reactive_power = selec_em4m_meter_get_float(SELEC_REACTIVE_POWER * 2, MULTIPLY_THOUSAND_UNIT);
  float apparent_power = selec_em4m_meter_get_float(SELEC_APPARENT_POWER * 2, MULTIPLY_THOUSAND_UNIT);
  float voltage = selec_em4m_meter_get_float(SELEC_VOLTAGE * 2, NO_DEC_UNIT);
  float current = selec_em4m_meter_get_float(SELEC_CURRENT * 2, NO_DEC_UNIT);
  float power_factor = selec_em4m_meter_get_float(SELEC_POWER_FACTOR * 2, NO_DEC_UNIT);
  float frequency = selec_em4m_meter_get_float(SELEC_FREQUENCY * 2, NO_DEC_UNIT);
  float maximum_demand_active_power =
      selec_em4m_meter_get_float(SELEC_MAXIMUM_DEMAND_ACTIVE_POWER * 2, MULTIPLY_THOUSAND_UNIT);
  float maximum_demand_reactive_power =
      selec_em4m_meter_get_float(SELEC_MAXIMUM_DEMAND_REACTIVE_POWER * 2, MULTIPLY_THOUSAND_UNIT);
  float maximum_demand_apparent_power =
      selec_em4m_meter_get_float(SELEC_MAXIMUM_DEMAND_APPARENT_POWER * 2, MULTIPLY_THOUSAND_UNIT);
...
void Selecem4mMeter::update() { this->send(MODBUS_CMD_READ_IN_REGISTERS, 0, MODBUS_REGISTER_COUNT); }
void Selecem4mMeter::dump_config() {
